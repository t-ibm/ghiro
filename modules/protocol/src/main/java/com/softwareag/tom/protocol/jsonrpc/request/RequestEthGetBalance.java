 ...
 * {@code eth_getBalance}.
 */
public class RequestEthGetBalance extends Request<ParamsAddress<String>, ResponseEthGetBalance> {
    public RequestEthGetBalance(Service jsonRpcService, Types.RequestEthGetBalance msg) {
        super(jsonRpcService, "eth_getBalance", new ParamsAddress<>(msg.getAddress(), msg.getBlock().getHeight().isEmpty() ? msg.getBlock().getState().name() : msg.getBlock().getHeight().toStringUtf8()));
    }
}